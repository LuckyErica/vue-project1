  ...
  .img-box {
    ...
    .img-inner-box {
      ...
      .img-info {
        ...
        p{
width:100%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:0.32rem;
        }
      }
    }
  }
...
<!-- —————————————↓HTML————————分界线———————————————————————— -->
<template>
<div class="vue-waterfall-easy" :style="isMobile? '':{width:colWidth*columnCount/100+'rem',left:'50%',marginLeft: -1*colWidth*columnCount/100 +'rem'}">
  <a class="img-box" v-for="(v,i) in imgsArrC" @click="previewImg(i)" :style="{width: isMobile ? '' : colWidth/50+'rem'}">
    <div class="img-inner-box">
      <div class="img-wraper" :style="{width:'100%'}">
        <img :src="v.src" style="display:block"/>
      </div>
      <div class="img-info">
        <p>{{v.info}}</p>
      </div>
  ...
  methods: {
    ...
    waterfall() { // 执行瀑布布局
      for (var i = this.beginIndex; i < this.imgsArr.length; i++) {
        ...
        this.imgBoxEls[i].style.left = minIndex * width/100 + 'rem'
        this.imgBoxEls[i].style.top = minHeight/100 + 'rem'
